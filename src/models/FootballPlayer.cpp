...
FootballPlayer::FootballPlayer(const StringBasic &name, const Date &birthdate,
    ...
    : name(name), birthdate(birthdate), number(number), position(position),
      goals(0), games(0), rating(rating), injured(false) {}
Clonable *FootballPlayer::Clone() const { return new FootballPlayer(*this); }
void FootballPlayer::Play(int goals) {
  this->goals += goals;
  this->games++;
}
void FootballPlayer::Injure() { injured = true; }
void FootballPlayer::UnInjure() { injured = false; }
void FootballPlayer::RatingUp(int x) { rating += x; }
void FootballPlayer::RatingDown(int x) { rating -= x; }
const StringBasic &FootballPlayer::Name() const { return name; }
const Date &FootballPlayer::Birthdate() const { return birthdate; }
FootballPlayer::PositionType FootballPlayer::Position() const {
  return static_cast<PositionType>(position);
}
int FootballPlayer::Number() const { return number; }
int FootballPlayer::Goals() const { return goals; }
int FootballPlayer::Games() const { return games; }
int FootballPlayer::Rating() const { return rating; }
bool FootballPlayer::Injured() const { return injured; }
