...
Date::Date(const Date &other) : count(other.count) {}
Date::Date(int year, int month, int day) : count(Count(year, month, day)) {}
Date::Date(int year, int dayNumber)
    : count(FirstDayOfYear(year) + dayNumber - 1) {}
Date::Date(int year, int week, WeekdayType weekday) : count(0) {
...
int Date::Century() const {
    return firstCentury + count / fourCenturies * 4 +
...
int Date::Year() const {
    ...
    return FirstYearOfCentury(c) + days / fourYears * 4 +
           Min(days % fourYears / 365, 3);
}
...
int Date::WeekYear() const {
    ...
    if (result != sunday.Year() &&
        Date(result + 1, 1, 1).Weekday() <= THURSDAY) {
        result++;
    }
    return result;
}
...
void Date::Read(std::istream &input) {
    ...
    this->count = Count(year, month, day);
}
...
int Date::FirstDayOfMonth(int y, int m) {
    bool leap = LeapYear(y);
    return FirstDayOfYear(y) +
           (accumulated[leap][m - 1] - daysInMonth[leap][m - 1]);
}

int Date::FirstDayOfYear(int y) {
    return FirstDayOfCentury(CenturyOf(y)) + (y - 1) % 100 / 4 * fourYears +
           (y - 1) % 4 * 365;
}

int Date::FirstDayOfCentury(int c) {
    return (c - firstCentury) / 4 * fourCenturies +
           (c - firstCentury) % 4 * oneCentury;
}
